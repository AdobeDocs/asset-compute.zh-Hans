...
模拟文件是JSON格式的http响应。 有关详细信息，请参阅[此文档](https://www.mock-server.com/mock_server/creating_expectations.html)。 如果要模拟多个主机名，请定义多个`mock-<mocked-host>.json`文件。 以下是名为`google.com`的`mock-google.com.json`的示例模拟文件：
...
错误测试用例不应包含预期的`rendition.*`文件，应在`errorReason`文件中定义预期的`params.json`。

>[!NOTE]
>
>如果测试用例不包含预期的`rendition.*`文件，并且未在`errorReason`文件中定义预期的`params.json`，则假定它是包含任何`errorReason`的错误用例。
...
查看[Asset Compute错误原因](https://github.com/adobe/asset-compute-commons#error-reasons)的完整列表和描述。

## 调试自定义应用程序 {#debug-custom-worker}

以下步骤显示如何使用Visual Studio Code调试自定义应用程序。 它允许查看实时日志、点击断点和逐步执行代码，以及在每次激活时实时重新加载本地代码更改。

`aio`现成可自动完成其中的许多步骤。 转到[Adobe Developer App Builder文档](https://developer.adobe.com/app-builder/docs/get_started/app_builder_get_started/first-app#)中的“调试应用程序”部分。 目前，以下步骤包括一个解决方法。

1. 从GitHub和可选的[ngrok](https://github.com/apache/openwhisk-wskdebug)安装最新的[wskdebug](https://www.npmjs.com/package/ngrok)。
...
1. 在JSON文件中添加用户设置。 它继续使用旧的Visual Studio代码调试器。 新环境的wskdebug [存在](https://github.com/apache/openwhisk-wskdebug/issues/74)一些问题`"debug.javascript.usePreview": false`。
1. 关闭通过`aio app run`打开的任何应用实例。
1. 使用`aio app deploy`部署最新的代码。
1. 仅使用`aio asset-compute devtool`运行Asset Compute Devtool。 保持打开。
1. 在Visual Studio代码编辑器中，将以下调试配置添加到您的`launch.json`：
   ...
   从`ACTION NAME`的输出中获取`aio app deploy`。

1. 从运行/调试的配置中选择`wskdebug worker`并按播放图标。 等待它启动，直到它在&#x200B;**[!UICONTROL 调试控制台]**&#x200B;窗口中显示&#x200B;**[!UICONTROL 准备激活]**。
